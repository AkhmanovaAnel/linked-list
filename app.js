...
class LinkedList {
  ...
  insert(position, value) {
    ...
    //добавление на самое первое место новый элемент
    if (position === 0) {
      node.next = this.head;
      this.head = node;
    } else {
  ...
  removeAt(position) {
    ...
    let current = this.head; //запоминаем указатель текущего элемента

    if (position === 0) {
      this.head = current.next; //если хотим удалить самый первый элемент нашего списка
    } else {
      let prev = null;
      let index = 0;
